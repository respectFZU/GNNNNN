...
def custom_loss(output, shortest_path, target_node, G):
    ...
    end_node_loss = 0.0
    predicted_end_node = output[shortest_path[-1]].argmax().item()
    if predicted_end_node != target_node:
        end_node_loss = 5.0

    total_loss = path_loss + 0.5 * neighbor_loss + end_node_loss
    return total_loss
...
for epoch in range(num_epochs):
    total_loss = 0
    count = 0
    for start_node in tqdm(range(1, G.number_of_nodes() + 1), desc=f"Epoch {epoch + 1}"):
        for target_node in range(1, G.number_of_nodes() + 1):
            ...
            try:
                ...
                loss = custom_loss(output, shortest_path, target_node, G)
...
end_not_reached_count = 0

for start_node in range(1, G.number_of_nodes() + 1):
    for target_node in range(1, G.number_of_nodes() + 1):
        if start_node == target_node:
            continue
...
print(f"Number of paths where end not reached: {end_not_reached_count}")